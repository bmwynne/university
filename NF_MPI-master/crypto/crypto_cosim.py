...
def crypto(out_data, out_ctrl, out_wr, out_rdy, 
           ...
           clk, reset):
    cmd = "iverilog -o crypto -c crypto.txt" 
    os.system(cmd)
    return Cosimulation("vvp -m ./myhdl.vpi crypto", 
...
#            sys.stdout.write(chr(dataout[8:0]))
    ...
    up = crypto(out_data, out_ctrl, out_wr, out_rdy,
                in_data, in_ctrl, in_wr, in_rdy,
                reg_req_in, reg_ack_in, reg_rd_wr_L_in, reg_addr_in, reg_data_in, reg_src_in,
                reg_req_out, reg_ack_out, reg_rd_wr_L_out, reg_addr_out, reg_data_out, reg_src_out,
                clk, reset)
