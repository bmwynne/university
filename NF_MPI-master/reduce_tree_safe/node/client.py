...
def barrier():
    try:  
        ...
        if root == rank and rank == 0:
            ...
            print '4'
            pkt = make_IP_pkt2(dst_MAC=DA, src_MAC=SA, TTL=TTL, dst_IP=dst_ip,
                               src_IP=src_ip, proto=proto, pkt_len=length)/scapy.UDP(sport=45329)/pack('2sHBB','\0A',123,1,6)/pack('HHHHHHHH',rank_n,root_n,size_n,0,768,0,0,0)/pack('3Q',socket.htonl(1),socket.htonl(2),socket.htonl(3))
        elif rank > root:
            print '5'
            pkt = make_IP_pkt2(dst_MAC=DA, src_MAC=SA, TTL=TTL, dst_IP=dst_ip,
